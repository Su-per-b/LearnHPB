...
class JsonConfig:
    ...
    def getFolderList(self, nodeName):
        return self.getList(nodeName, 'folderList')

    def getFileList(self, nodeName):
        return self.getList(nodeName, 'fileList')

    def getList(self, nodeName, listName):
        node = self.root[nodeName]

        fileList = node[listName]
    ...
    def getMinifiedOutputFile(self):
        ...
        fileName = "temp\\min\\%s\\%s.min.%s" % (outputFileExtension, outputFileNameBase,outputFileExtension)
        return str(fileName)
