...
lgb.view.LightingView.prototype.onChange_lightingType_ = function(lightingType) {
  this.buildGrid_();
};
...
lgb.view.LightingView.prototype.onSceneLoaded_ = function() {
  ...
  for (var i = this.scene_.children.length - 1; i >= 0; i--) {
    ...
    if (mesh.name == 'recessed') {
      ...
      this.gridRecessed = new lgb.model.GridModel(this.appData_.gridRecessed, mesh.geometry.getDimensions());

    } else if (mesh.name == 'pendant') {

      this.pendantGeom = mesh.geometry;
      this.gridPendant = new lgb.model.GridModel(this.appData_.gridPendant, mesh.geometry.getDimensions());
    }
...
lgb.view.LightingView.prototype.buildGrid_ = function() {
  ...
  if (this.dataModel.lightingType == lgb.model.LightingModel.Type.PENDANT) {

    geometry = this.pendantGeom;
    gridModel = this.gridPendant;

  } else if (this.dataModel.lightingType == lgb.model.LightingModel.Type.RECESSED) {
