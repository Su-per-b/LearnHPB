			...
			if ( xhr.status == 200 || xhr.status == 0 ) {
				...
				try {
					...
					if ( json.metadata === undefined || json.metadata.formatVersion === undefined || json.metadata.formatVersion !== 3 ) {
						...
						console.error( 'Deprecated file format. ' + xhr.responseText);
						return;
					...
					console.error( error );
					console.warn( "DEPRECATED: [" + url + "] seems to be using old model format: " +  + xhr.responseText );
...
THREE.SceneLoaderEx.prototype.createScene = function ( json, callbackFinished, url ) {
	...
	for ( dg in data.geometries ) {
        ...
        if (json.metadata !== undefined) {
          modelJson.metadata = json.metadata;
        };
