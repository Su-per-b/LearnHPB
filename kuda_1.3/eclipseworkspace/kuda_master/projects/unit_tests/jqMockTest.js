...
jqUnit.test("without shortcut", function() {    
...
jqUnit.test("mocking native dialog", function() {
	...
	jqUnit.ok(b1, "should be true");
	var b2 = confirm("are you sure 2?");
	jqUnit.ok(!b2, "should be true (2)");	
...
jqUnit.test("args() expectation", function() {
...
jqUnit.test("Expression is()", function() {
	var myobj = {fn:function(){}};
	var mock = new jqMock.Mock(myobj, "fn");
...
jqUnit.test("expectation resolution", function() {
	var myobj = {fn:function(){}};
	var mock = new jqMock.Mock(myobj, "fn");
	...
	mock.modify().multiplicity(expect.atLeast(2)).args("x");
	for(var i=0; i<4; i++) myobj.fn("x");
	mock.verifyAll();
	mock.reset();
	
	mock.modify().multiplicity(expect.times(2,3)).args("x");
	mock.modify().multiplicity(expect.exactly(1)).args(is.not("y"));
	mock.modify().multiplicity(expect.atMost(1)).args("x");
...
jqUnit.test("unexpected invocation, but using verify() only", function() {
	var myobj = {fn:function(){}};
	var mock = new jqMock.Mock(myobj, "fn");	
	mock.modify().args(1);
	mock.modify().args(2);
...
jqUnit.test("returnValue and intercepting", function() {
	...
	jqUnit.equals(myobj.sum(5,6) , 100);
	jqUnit.equals(myobj.sum(5,6) , 11);
	mock.restore();
	jqUnit.equals(myobj.sum(11,12) , 23);
});

jqUnit.test("throwException", function() {
	jqUnit.expect(2);
	var myobj = {fn:function(){}};
	var mock = new jqMock.Mock(myobj, "fn");
	...
	try {
		...
		jqUnit.equals(ex, "my exception");
	}
	try {
		myobj.fn(1);
	} catch (ex) {
		jqUnit.equals(ex.message, "some error");
	}	
});

jqUnit.test("multiplicity", function() {
	var myobj = {fn:function(){}};
	var mock = new jqMock.Mock(myobj, "fn");

	mock.modify().args().multiplicity(expect.exactly(1));
	mock.modify().args('a').multiplicity(expect.times(3));
	mock.modify().args('c').multiplicity(2);
	mock.modify().args('b','bbb').multiplicity(expect.times(2,3));
	mock.modify().args({p:'p'},[7,8]);
	mock.modify().args('y').multiplicity(expect.atLeast(1));
	mock.modify().args('z').multiplicity(expect.atMost(4));	
	mock.modify().args('foo').multiplicity(expect.atMost(0));
	mock.modify().args('zero1').multiplicity(0);
	mock.modify().args('zero2').multiplicity(expect.exactly(0));
	mock.modify().args('zero3').multiplicity(expect.times(0,2));
...
jqUnit.test("tostring", function() {
	jqUnit.equals(jqMock.tostring(3), "3");
	jqUnit.equals(jqMock.tostring("a"), "'a'");
	jqUnit.equals(jqMock.tostring([]), "[]");
	jqUnit.equals(jqMock.tostring([1,2,3]), "[1,2,3]");
	jqUnit.equals(jqMock.tostring([1,2,"x"]), "[1,2,'x']");	
	jqUnit.equals(jqMock.tostring(function(){}), "function()");
	jqUnit.equals(jqMock.tostring({}), "{}");
	jqUnit.equals(jqMock.tostring({a:1,b:2,c:'cc'}), "{a:1,b:2,c:'cc'}");
	var x;
	jqUnit.equals(jqMock.tostring(x), "undefined");
	jqUnit.equals(jqMock.tostring(null), "null");

	jqUnit.equals(jqMock.tostring(is.anything), "is.anything");
	jqUnit.equals(jqMock.tostring(is.not('x')), "is.not('x')");
	jqUnit.equals(jqMock.tostring(is.instanceOf(Array)), "is.instanceOf(Array)");
	jqUnit.equals(jqMock.tostring(is.objectThatIncludes({a:'a'})), "is.objectThatIncludes({a:'a'})");
	jqUnit.equals(jqMock.tostring(is.regex(/^[0-9]*/)), "is.regex(/^[0-9]*/)");
	jqUnit.equals(jqMock.tostring(is.anyOf([is.not('a'), is.not('b')])), "is.anyOf([is.not('a'),is.not('b')])");
	jqUnit.equals(jqMock.tostring(is.allOf([is.not(null), is.not(undefined)])), "is.allOf([is.not(null),is.not(undefined)])");
	jqUnit.equals(jqMock.tostring(is.exception({name: "ReferenceError", message: "blah is not defined", type: ReferenceError})), "is.exception({name:'ReferenceError',message:'blah is not defined',type:is.instanceOf(ReferenceError)})");
	jqUnit.equals(jqMock.tostring(is.exception("foo")), "is.exception({message:'foo'})");
	...
	jqUnit.equals(jqMock.tostring(obj), objStr);
});

jqUnit.test("circular reference", function() {
	var a={}; 
	var b={};
	var x1={'x':a};
	var x2={'x':b};
	...
	jqUnit.equals(jqMock.tostring(a), "{x:{x:...}}");
	jqUnit.equals(jqMock.tostring(aa), "[[...]]");
		
	try {
		jqMock.assertThat(a,b);
	} catch (ex) {
		jqUnit.equals(ex.message, "Circular Reference detected in the object!", "ciruclar ref in object");
	}
	try {
		jqMock.assertThat(a,b);
	} catch (ex) {
...
jqUnit.test("object assertion", function() {
...
jqUnit.test("expectThatExceptionThrown", function() {
	// simple compare
...
jqUnit.test("error handling", function() {
	jqUnit.expect(15);
	var obj = {x:'x'};
	try {
		new jqMock.Mock(obj, "x");
	} catch(ex) {
		jqUnit.equals(ex.message, "Could not create Mock: x is not a function", "exception should be thrown");
	}
	var obj2;
	try {
		new jqMock.Mock(obj2, "x");
	} catch(ex) {
		jqUnit.equals( ex.message, "Could not create Mock: the object or function you specified is not defined, obj.x", "exception should be thrown");
	}	
	var obj3 = {};
	try {
	...
	try {
		...
		jqUnit.equals(ex.message, "Expectation setup error. You must set up some expectations", "exception should be thrown");
	}
	var alertMock = new jqMock.Mock(window, "alert");
	try {		
		jqUnit.ok(window.alert.isMock, "flag should be set");
		new jqMock.Mock(window, "alert");		
	} catch(ex) {
		jqUnit.equals(ex.message, "Could not create Mock: you tried to overwrite a function which is already a mock!", "exception should be thrown");
	}
	alertMock.restore();
	...
	try {
		...
		jqUnit.equals(ex.message, "Invalid argument to is.instanceOf() , argument must be a Class", "exception should be thrown");
	}
	try {
		mock5.modify().args(is.regex('x'));
	} catch(ex) {
		jqUnit.equals(ex.message, "Invalid argument to is.regex() , argument must be regex", "exception should be thrown");
	}
	try {
		mock5.modify().args(is.anyOf('x'));
	} catch(ex) {
		jqUnit.equals(ex.message, "Invalid argument to is.anyOf() , argument must be an Array", "exception should be thrown");
	}
	try {
		mock5.modify().args(is.allOf('x'));
	} catch(ex) {
		jqUnit.equals(ex.message, "Invalid argument to is.allOf() , argument must be an Array", "exception should be thrown");
	}
	try {
		mock5.modify().args(is.custom('x'));
	} catch(ex) {
		jqUnit.equals(ex.message, "Invalid argument to is.custom() , argument must be a Function that returns a boolean value", "exception should be thrown");
	}
	try {
		mock5.modify().multiplicity("x");
	} catch(ex) {
		jqUnit.equals(ex.message, "Invalid argument for multiplicity()", "exception should be thrown");
	}
		
	try {
		jqMock.addShortcut();
	} catch(ex) {
		jqUnit.equals(ex.message, "Cannot add shortcuts, expect() or is() already exists in global scope!", "exception should be thrown");	
	}	

	try {
		is.exception(0);
	} catch(ex) {
		jqUnit.equals(ex.message, "Invalid argument to is.exception() , argument must be an object or string");
	}
	
	try {
		is.exception({type:'foo'});
	} catch(ex) {
		jqUnit.equals(ex.message, "Invalid argument to is.exception() , the type attribute of the parameter must be a Class");
	}
});
...
jqUnit.test("expectation resolution", function() {
	...
	mock.modify().multiplicity(expect.atLeast(2)).args("x"); // atLeast will always match...
...
jqUnit.test("using verify() does not print out unexpected section", function() {
	var myobj = {fn:function(){}};
	var mock = new jqMock.Mock(myobj, "fn");
...
jqUnit.test("multiplicity", function() {
	...
	mock.modify().args("foobar").multiplicity(expect.atLeast(3));
	mock.modify().args("foo").multiplicity(expect.atMost(1));
	mock.modify().args('a','b', 'c').multiplicity(expect.times(2,3));
